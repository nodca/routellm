...
fn draw_help_modal(frame: &mut Frame) {
    ...
    let text = vec![
        ...
        Line::from(""),
        Line::from("导航"),
        Line::from("/       过滤左侧 routes"),
        Line::from("Left/Right 在面板之间切换"),
        Line::from("Home/End 跳到顶部 / 底部"),
        Line::from("PgUp/PgDn 在长列表中快速翻页"),
        Line::from("Up/Down 移动选中项"),
        Line::from(""),
        Line::from("操作"),
        Line::from("a       按当前面板新增 route 或 channel"),
        Line::from("i       编辑当前 channel 的 base_url / key / 路由参数"),
        Line::from("x       删除空 route 或当前 channel"),
        Line::from("t       测活当前 channel"),
        Line::from("T       测活当前 route 下全部 channel"),
        Line::from("Space   切换当前 channel 状态"),
        Line::from("e       启用当前 channel"),
        Line::from("d       禁用当前 channel"),
        Line::from("c       恢复当前 channel，清掉冷却 / 阻断"),
        Line::from("u       复制下游 base url（全局）"),
        Line::from("K       复制已配置的 auth key（全局）"),
        Line::from("Enter   进入下一层或打开详情"),
        Line::from("Enter/y 确认当前操作"),
        Line::from("Esc/n   取消当前操作"),
        Line::from(""),
        Line::from("应用"),
        Line::from("r       刷新全部数据"),
        Line::from("?       显示 / 关闭帮助"),
